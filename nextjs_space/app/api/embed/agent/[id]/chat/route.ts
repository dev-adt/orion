...
const KNOWLEDGE_BUDGET = 24000;

function getBaseUrl() {
  return (process.env.NEXTAUTH_URL || 'https://orion.abacusai.app').replace(/\/$/, '');
}
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!agent || !agent.active || !agent.embeddable) {
      return NextResponse.json({ error: 'Agent not found or not embeddable' }, { status: 404 });
    }
    ...
    sections.push(agent.systemPrompt?.trim() || 'Bạn là một trợ lý AI chuyên nghiệp.');
    ...
    if (!upstream.ok || !upstream.body) {
      ...
      console.error('Embed agent chat upstream error:', upstream.status, txt.slice(0, 300));
    ...
    console.error('Embed agent chat error:', error);
